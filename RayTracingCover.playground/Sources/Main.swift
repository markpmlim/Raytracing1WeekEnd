
import Cocoa
import simd

struct Pixel {
    var red: UInt8
    var green: UInt8
    var blue: UInt8
}

public func cgImageFromRawBitmap(_ bitMapData: UnsafeRawPointer,
                                 _ width: Int,
                                 _ height: Int) -> CGImage? {
    let bitsPerComponent: Int = 8
    let bytesPerPixel = 3
    let colorSpace = NSColorSpaceName.deviceRGB
    let bir = NSBitmapImageRep(bitmapDataPlanes: nil,
                               pixelsWide: width,
                               pixelsHigh: height,
                               bitsPerSample: bitsPerComponent,
                               samplesPerPixel: bytesPerPixel,
                               hasAlpha: false,
                               isPlanar: false,
                               colorSpaceName: colorSpace,
                               bytesPerRow: width*bytesPerPixel,
                               bitsPerPixel: bitsPerComponent*bytesPerPixel)
    guard let bmImageRep = bir
    else {
        return nil
    }
    memcpy(bmImageRep.bitmapData,
           bitMapData,
           width*height*bytesPerPixel)
    return bmImageRep.cgImage
}


func rayColor(_ ray: Ray, _ world: HittableList, _ depth: Int) -> color {
    // We need to instantiate a (zeroed) HitRecord.
    var rec = HitRecord()

    // If we've exceeded the ray bounce limit, no more light is gathered.
    if depth < 0 {
        return color(0,0,0)
    }

    // Fixing Shadow Acne
    if world.isHitBy(ray, min: 0.001, max: Double.infinity, &rec) {
        var scattered = ray
        var attenuation = color(0)
        if rec.material.scatter(ray, rec, &attenuation, &scattered) {
            return attenuation * rayColor(scattered, world, depth-1)
        }
        else {
            return color(0)
        }
    }
    else {
        // background
        let unitDirection = normalize(ray.direction)
        let t = 0.5*(unitDirection.y + 1.0)
        return (1.0-t)*color(1.0, 1.0, 1.0) + t*color(0.5, 0.7, 1.0)
    }
}

func random_scene() -> HittableList {
    var objects = [Hittable]()
    let materialGround = Lambertian(albedo: color(0.5, 0.5, 0.5))
    let ground = Sphere(center: point3(0.0, -1000, 0.0), radius: 1000,
                        material: materialGround)
    objects.append(ground)
    for a in -11..<11 {
        for b in -11..<11 {
            let chooseMaterial = random_double()
            let center = point3(Double(a) + 0.9 * random_double(),
                                0.2,
                                Double(b) + 0.9 * random_double())
            if length(center - point3(4, 0.2, 0)) > 0.9 {
                if chooseMaterial < 0.8 {
                    // diffuse
                    let albedo = random() * random()
                    let sphereMaterial = Lambertian(albedo: albedo)
                    objects.append(Sphere(center: center,
                                          radius: 0.2,
                                          material: sphereMaterial))
                }
                else if chooseMaterial < 0.95 {
                    // metal
                    let albedo = random(0.5, 1.0)
                    let fuzz = random_double(0.0, 0.5)
                    let sphereMaterial = Metal(albedo: albedo, fuzz: fuzz)
                    objects.append(Sphere(center: center,
                                          radius: 0.2,
                                          material: sphereMaterial))
                }
                else {
                    // glass
                    let sphereMaterial = Dielectric(1.5)
                    objects.append(Sphere(center: center,
                                          radius: 0.2,
                                          material: sphereMaterial))
                }
            }
        }
    }

    let material1 = Dielectric(1.5)
    objects.append(Sphere(center: point3(0, 1, 0),
                          radius: 1.0,
                          material: material1))

    let material2 = Lambertian(albedo: color(0.4, 0.2, 0.1))
    objects.append(Sphere(center: point3(-4, 1, 0),
                          radius: 1.0,
                          material: material2))

    let material3 = Metal(albedo: color(0.7, 0.6, 0.5), fuzz: 0.0)
    objects.append(Sphere(center: point3(4, 1, 0),
                          radius: 1.0,
                          material: material3))
    let world = HittableList(objects)

    return world
}


// Listing 70: use smaller values for faster image generation
public func generateCGImage() -> CGImage? {

    // Image
    let aspectRatio: Double = 3.0/2.0
    let imageWidth = 400            //1200
    let imageHeight = Int(Double(imageWidth)/aspectRatio)
    let samples_per_pixel = 50      //500
    let max_depth = 50

    // World

    let world = random_scene()

    // Camera - Scene camera with depth-of-field
    let look_from = point3(13, 2, 3)
    let look_at = point3(0, 0, 0)
    let v_up = vec3(0, 1, 0)
    let dist_to_focus = 10.0
    let aperture = 0.1
    let cam = Camera(look_from,
                     look_at,
                     v_up,
                     20,
                     aspectRatio,
                     aperture,
                     dist_to_focus)

    let pixel = Pixel(red: 0, green: 0, blue: 0)
    var bitMap = [Pixel](repeating: pixel,
                         count: imageWidth * imageHeight)

    // Render

    DispatchQueue.concurrentPerform(iterations: imageHeight) {
        row in
        DispatchQueue.concurrentPerform(iterations: imageWidth) {
            col in
            var pixelColor = color(0)
            DispatchQueue.concurrentPerform(iterations: samples_per_pixel) {
                dummy in
                let u = (Double(col) + random_double()) / Double(imageWidth-1)
                let v = (Double(row) + random_double()) / Double(imageHeight-1)
                let ray = cam.getRay(double2(u,v))
                pixelColor += rayColor(ray, world, max_depth)
            }
            // Divide the color by the number of samples ...
            pixelColor /= Double(samples_per_pixel)
            //  ... and gamma-correct for gamma=2.0
            pixelColor = color(sqrt(pixelColor.x), sqrt(pixelColor.y), sqrt(pixelColor.z))

            pixelColor = clamp(pixelColor, min: 0.0, max: 0.999)
            let red = UInt8(256*pixelColor.x)
            let green = UInt8(256*pixelColor.y)
            let blue = UInt8(256*pixelColor.z)
            let pixel = Pixel(red: red,
                              green: green,
                              blue: blue)
            bitMap[imageWidth*(imageHeight-row-1) + col] = pixel
        }
    }
    return cgImageFromRawBitmap(bitMap, imageWidth, imageHeight)
}
